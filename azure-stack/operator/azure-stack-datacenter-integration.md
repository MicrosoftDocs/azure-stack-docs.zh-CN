...
评估 Azure Stack 解决方案的采购时，必须在硬件配置方面做出选择，因为它直接影响到 Azure Stack 解决方案的总体容量。 考虑因素包括 CPU、内存密度、存储配置和总体解决方案规模（例如服务器的数目）的一般选择。 不同于传统的虚拟化解决方案，简单地评估这些组件并不能很好地确定可用的容量。 第一个原因是 Azure Stack 构建为在解决方案自身内部托管基础结构或管理组件。 第二个原因是容量的某些解决方案容量的保留用于支持复原;中的租户工作负荷的中断降到最低的方式的解决方案的软件更新。 
...
当需要更高级别的访问权限时不会通过警报中介步骤解决的问题进行疑难解答时，必须咨询 Microsoft 支持部门。 支持人员会提供暂时性的完整管理员访问权限，让你通过某种方法访问系统，以执行更高级的操作。 

## <a name="identity-considerations"></a>标识注意事项

### <a name="choose-identity-provider"></a>选择标识提供者
需要考虑用于 Azure Stack 部署的标识提供者：Azure AD 或 AD FS。 而无需重新部署整个系统的部署后，无法切换标识提供程序。 如果你未拥有 Azure AD 帐户且使用的是云服务提供商提供给你的帐户，并且你决定切换提供商并使用其他 Azure AD 帐户，则此时你必须联系解决方案提供商让其根据你的成本为你重新部署解决方案。
...
你将需要考虑你想要规划你的 Azure Stack 命名空间，尤其是区域名称和外部域名。 公开终结点的 Azure Stack 部署的外部完全限定域名 (FQDN) 由以下两个名称组成：&lt;*区域*&gt;.&lt;*fqdn*&gt;。 例如 *east.cloud.fabrikam.com*。 在此示例中，Azure Stack 门户将在以下 URL 中提供：
...
| 名称 | 描述 | 
| -------- | ------------- | 
|区域名称 | 第一个 Azure Stack 区域名称。 此名称用作 Azure Stack 管理的公共虚拟 IP 地址 (VIP) 的 FQDN 的一部分。 通常，区域名称是一个物理位置标识符，例如数据中心位置。<br><br>区域名称必须仅包含字母和 0-9 之间的数字。 没有特殊字符如"-"或允许使用"#"，等等。| 
...
对于部署，你将需要提供面向公众的终结点的安全套接字层 (SSL) 证书。 概括而言，证书具有以下要求：
...
对于混合云方案，你将需要规划你想要将 Azure Stack 连接到 Azure。 支持使用两种方法将 Azure Stack 中的虚拟网络连接到 Azure 中的虚拟网络： 
...
- **出站 NAT**。 默认情况下，Azure Stack 中的所有虚拟机都可通过出站 NAT 连接到外部网络。 在 Azure Stack 中创建的每个虚拟网络分配有一个公共 IP 地址。 无论虚拟机是直接被分配了公共 IP 地址，还是位于采用公共 IP 地址的负载均衡器后面，都可以使用虚拟网络的 VIP，通过出站 NAT 进行出站访问。 此方法只适用于虚拟机发起的、目标为外部网络（Internet 或 Intranet）的通信， 它不能用于与来自外部的虚拟机进行通信。

### <a name="hybrid-connectivity-options"></a>混合连接选项

混合连接，务必要考虑哪种部署你想要提供和部署位置。 你将需要考虑您是否需要隔离每个租户的网络流量以及是否要进行 intranet 或 internet 部署。

- **单租户 Azure Stack**。 Azure Stack 部署看起来，至少从网络的角度看，如同它是一个租户。 可能存在许多租户订阅，但如同任何 Intranet 服务一样，所有流量通过相同的网络传输。 从一个订阅的网络流量通过与另一个订阅相同的网络连接传输，并且不需要通过加密的隧道隔离。

- **多租户 Azure Stack**。 Azure Stack 部署到 Azure Stack 外部的网络的每个租户订阅的流量必须独立于其他租户的网络流量。
 
- **Intranet 部署**。 位于企业 Intranet 上的 Azure Stack 部署，通常位于专用 IP 地址空间中，并在一个或多个防火墙后面。 公共 IP 地址不是真正面向公众，因为它们不能直接通过公共 internet 路由。

- **Internet 部署**。 Azure Stack 部署连接到公共 internet，并使用 internet 路由的公共 IP 地址的公共 VIP 范围中。 部署仍可位于防火墙后面，但可以从公共 Internet 和 Azure 直接访问公共 VIP 范围。
...
- 你将需要在现有的基于 Windows 的文件服务器或第三方设备上的外部 SMB 文件共享。
- 应将此同一共享用于网络交换机与硬件生命周期主机的备份。 OEM 硬件供应商会帮助提供这些组件的备份和还原指导，因为这些操作在 Azure Stack 外部进行。 你要负责运行备份的工作流根据 OEM 供应商的建议。
