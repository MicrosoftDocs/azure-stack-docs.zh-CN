...
通过连接的部署，你可以使用管理员门户下载 marketplace 项。
...
如果 Azure Stack 集线器限制或没有 internet 连接，则可以使用 PowerShell 和*marketplace 联合工具*将 marketplace 项下载到具有 internet 连接的计算机。 然后，将这些项传输到 Azure Stack Hub 环境。 在离线环境中，无法使用 Azure Stack Hub 门户下载市场项。
