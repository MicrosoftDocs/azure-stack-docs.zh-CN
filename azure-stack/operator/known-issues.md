...
- 适用：此问题适用于所有受支持的版本。
- 原因：在用户门户中，存储帐户的名称显示为**存储帐户-blob、文件、表、队列**，但 Azure Stack 不支持**文件**。
- 发生次数：常见
...
### <a name="capacity-mnitoring-in-sql-and-mysql-resource-provider-keeps-loading"></a>Mnitoring 在 SQL 和 MySQL 资源提供程序中的容量

- 适用：此问题适用于所有受支持的版本。
- 原因：当前版本的 SQL 和 MySQL 资源提供程序与1910更新中的某些最新门户更改不兼容。
- 修正：按照资源提供程序更新过程操作，应用 SQL 和 MySQL 资源提供程序修补程序1.1.47.0。 （[SQL RP 版本 1.1.47.0](https://aka.ms/azurestacksqlrp11470)和[MySQL RP 版本 1.1.47.0](https://aka.ms/azurestackmysqlrp11470)）。
- 发生次数：常见
...
- 适用：此问题适用于所有受支持的版本。
- 原因：无法更改 VM 的主 NIC。 删除/分离主 NIC 会导致启动 VM 时出现问题。
- 发生次数：常见
...
- 修正：请求操作员添加具有额外配额的附加计划。 编辑当前计划的配额将不起作用或反映增加的配额。
- 发生次数：罕见
