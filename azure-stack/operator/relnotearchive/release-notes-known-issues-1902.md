...
Azure Stack 1902 更新生成类型已 **满**。 有关更新内部版本类型的详细信息，请参阅[管理 Azure Stack 中的更新](../azure-stack-updates.md)一文。
...
> [!IMPORTANT]
> 你可以直接从 **1.1901.0.95** 或 **1.1901.0.99** 版本安装1902，无需首先安装任何1901修补程序。 但是，如果已安装旧版 **1901.2.103** 修补程序，则必须先安装新版 [1901.3.105 修补程序](https://support.microsoft.com/help/4495662)才能继续安装 1902。
...
- 在开始安装此更新之前，请使用以下参数运行 [Test-AzureStack](../azure-stack-diagnostic-test.md)，以验证 Azure Stack 的状态并解决发现的所有操作问题，包括所有警告和故障。 另外，请查看活动警报，并解决所有需要采取措施的警报。
  ...
  如果在 `AzsControlPlane` 执行 **test-azurestack** 时包含参数，你将在 **test-azurestack** 输出中看到以下失败： " **Azure Stack 控制平面网站摘要**"。 可以放心忽略此特定错误。

- 如果 Azure Stack 由 System Center Operations Manager (SCOM) 进行管理，请确保在应用1902之前将 [Microsoft Azure Stack 的管理包](https://www.microsoft.com/download/details.aspx?id=55184) 更新为版本1.0.3.11。
...
- 1902 修复了网络控制器 VSwitch 服务中的一个问题，即特定节点上的所有 VM 都脱机。  此问题导致其进入主丢失状态，在这种情况下，无法联系主副本，但角色尚未故障转移到另一个正常的实例，这种情况只能通过联系 Microsoft 支持服务来解决。
...
- 在用户门户中，如果尝试将 **后端池** 添加到 **负载均衡器**，则操作将失败，并出现错误消息 " **无法更新负载平衡器 ...**  "。 若要解决此问题，请使用 PowerShell、CLI 或 Azure 资源管理器模板将后端池与负载均衡器资源关联。

- 在用户门户中，如果尝试为 **负载均衡器** 创建 **入站 NAT 规则**，操作将失败，并出现错误消息 "**更新负载平衡器失败 ...** "。 若要解决此问题，请使用 PowerShell、CLI 或 Azure 资源管理器模板将后端池与负载均衡器资源关联。
