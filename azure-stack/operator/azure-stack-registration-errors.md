...
| BadCustomerSubscriptionId | 提供的**客户订阅标识符**和**注册名称**订阅标识符不属于同一 Microsoft CSP。 检查客户订阅标识符是否正确。 客户订阅 ID 区分大小写。 如果问题仍然存在，请联系支持部门。 | 如果客户订阅是 CSP 订阅，则会发生此错误，但会将其汇总到与初始注册中使用的订阅不同的 CSP 合作伙伴。 进行此项检查是为了防止发生这种情况，这会导致为 CSP 伙伴计费，而不负责使用 Azure Stack 中心。 |
