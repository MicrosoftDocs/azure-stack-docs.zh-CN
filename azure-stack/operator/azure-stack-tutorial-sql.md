---
title: 在 Azure Stack 中提供高度可用的 SQL 数据库
description: 了解如何使用 Azure Stack 创建 SQL Server 资源提供程序主机计算机和高度可用的 SQL AlwaysOn 数据库。
...
ms.locfileid: "72283357"
---
# <a name="offer-highly-available-sql-databases"></a>提供高度可用的 SQL 数据库

作为 Azure Stack 运算符，你可以将服务器 Vm 配置为承载 SQL Server 数据库。 成功创建 SQL 宿主服务器并通过 Azure Stack 管理该服务器后，订阅 SQL 服务的用户可以轻松地创建 SQL 数据库。

本文介绍如何使用 Azure Stack 快速入门模板创建[SQL Server AlwaysOn 可用性组](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/overview-of-always-on-availability-groups-sql-server?view=sql-server-2017)，将其添加为 Azure Stack SQL 宿主服务器，然后创建高度可用的 sql 数据库。
...
在开始之前，请确保已成功安装[SQL Server 资源提供程序](azure-stack-sql-resource-provider-deploy.md)，并且 Azure Stack marketplace 中提供以下各项：

> [!IMPORTANT]
> 要使用 Azure Stack 快速入门模板，需要满足以下所有要求。
...
使用本部分中的步骤，通过使用[SQL-2016-AlwaysOn Azure Stack 快速入门模板](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/sql-2016-alwayson)部署 SQL Server AlwaysOn 可用性组。 此模板在 Always On 可用性组中部署两个 SQL Server Enterprise、标准或开发人员实例。 它创建以下资源：
...
5. 在 "**自定义部署**" 边栏选项卡中，选择要使用的订阅并创建新的资源组，或选择现有的资源组进行自定义部署。<br><br> 接下来，选择资源组的位置（对于 ASDK 安装，请选择“本地”），然后单击“创建”。 将验证自定义部署设置，然后启动部署。
...
使用 SQL AlwaysOn 可用性组的负载均衡器的负载均衡器公共 IP 和 SQL 身份验证登录信息，Azure Stack 运算符现在可以[使用 Sql alwayson 可用性组创建 Sql 宿主服务器](azure-stack-sql-resource-provider-hosting-servers.md#provide-high-availability-using-sql-always-on-availability-groups)。 
...
通过 Azure Stack 操作员创建、配置并添加 SQL AlwaysOn 可用性组作为 Azure Stack SQL 宿主服务器后，具有订阅的租户用户（包括 SQL Server 数据库功能）可以创建支持的 SQL 数据库AlwaysOn 功能，请按照本部分中的步骤操作。 

> [!NOTE]
> 使用订阅提供 SQL Server 功能（Microsoft.sqladapter 服务）的租户用户身份从 Azure Stack 用户门户运行这些步骤。
...
3. 选择 " **SKU** "，然后选择要使用的相应 SQL 宿主服务器 SKU。 在此示例中，Azure Stack 运算符已创建**企业 HA** SKU，以支持 SQL AlwaysOn 可用性组的高可用性。
