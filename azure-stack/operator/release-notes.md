...
Azure Stack Hub 定期发布修补程序。 请确保在更新到2008之前安装最新的2005修补程序。 此外，从2005版开始，在更新到新的主版本 (例如1.2005 到 1.2008) 时，如果在包下载时有任何可用的修补程序，则会自动安装新的主要版本中的最新修补 (程序) 。 你的2008安装与所有修补程序都是最新的。 然后，如果为2008发布了修补程序，则应进行安装。
...
> [!TIP]
> 如果你希望收到有关每个修补程序版本的通知，请订阅 [**RSS 源**](https://azurestackhubdocs.azurewebsites.net/xml/hotfixes.rss) ，通知每个修补程序版本。

### <a name="after-successfully-applying-the-2008-update"></a>成功应用 2008 更新之后

由于 Azure Stack Hub 修补程序是累积的，因此，作为最佳做法，你应安装已为你的内部版本发布的所有修补程序，以确保主要版本之间的最佳更新体验。 当更新到新的主版本 (例如，1.2005 到 1.2008) 时，如果在包) 下载时有任何可用的修补程序，则会自动安装最新的修补 (程序。
...
- [主动日志收集和按需日志收集](./diagnostic-log-collection.md?preserve-view=true&view=azs-2002)所花费的时间已缩短 80%。 日志收集时间可能会比此预期值长，但不需要 Azure Stack Hub 操作员执行操作，除非日志收集失败。
...
::: moniker range="<azs-2002"
你可以在 " [**资源 > 发行说明存档**](./relnotearchive/release-notes-1910.md)" 下，访问左侧目录中 Azure Stack 集线器发行说明的早期版本。 这些存档文章仅供参考目的提供，并不意味着支持这些版本。 有关 Azure Stack Hub 支持的信息，请参阅 [Azure Stack Hub 服务策略](azure-stack-servicing-policy.md)。 如需进一步的帮助，请联系 Microsoft 客户支持服务。
::: moniker-end