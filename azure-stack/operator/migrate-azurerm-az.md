...
- 删除了以下属性：
  - 从 `PortalHostnameConfiguration` 中删除了 `ProxyHostnameConfiguration` 类型的属性 `ManagementHostnameConfiguration`、`ScmHostnameConfiguration`、`PsApiManagementHostnameConfiguration` 和 `PsApiManagementContext`。 请改用 `PortalCustomHostnameConfiguration` 类型的 `ProxyCustomHostnameConfiguration`、`ManagementCustomHostnameConfiguration`、`ScmCustomHostnameConfiguration` 和 `PsApiManagementCustomHostNameConfiguration`。
...
- 从 `InvoiceName` cmdlet 中删除了 `Get-AzConsumptionUsageDetail` 参数。  脚本将需要为发票使用其他标识参数。

### <a name="azcompute-previously-azurermcompute"></a>Az.Compute（以前的 AzureRM.Compute）

- 从 `IdentityIds` 和 `Identity` 对象中的 `PSVirtualMachine` 属性中删除了 `PSVirtualMachineScaleSet`。脚本不应当再使用此字段的值来做出处理决策。
- `InstanceView` 对象的 `PSVirtualMachineScaleSetVM` 属性的类型已从 `VirtualMachineInstanceView` 更改为 `VirtualMachineScaleSetVMInstanceView`
- 从 `AutoOSUpgradePolicy` 属性中删除了 `AutomaticOSUpgrade` 和 `UpgradePolicy` 属性
- `Sku` 对象中的 `PSSnapshotUpdate` 属性的类型已从 `DiskSku` 更改为 `SnapshotSku`
...
- 从 `PurgeDisabled`、`PSKeyVaultKeyAttributes` 和 `PSKeyVaultKeyIdentityItem` 对象中删除了 `PSKeyVaultSecretAttributes` 属性。脚本不再应该引用 ```PurgeDisabled``` 属性来做出处理决策。
