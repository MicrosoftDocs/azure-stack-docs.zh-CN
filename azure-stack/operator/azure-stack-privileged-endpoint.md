...
在开始针对集成系统的此过程之前，请确保可以通过 IP 地址或 DNS 访问 PEP。 初始部署 Azure Stack 后，只能通过 IP 地址访问 PEP，因为尚未设置 DNS 集成。 你的 OEM 硬件供应商将为你提供一个名为**AzureStackStampDeploymentInfo**的 JSON 文件，其中包含 PEP IP 地址。
...
3. 将 PEP 会话导入到本地计算机
   ...
   ```
4. 现在，你可以使用 tab 自动补全，并使用 PEP 的所有功能和 cmdlet 在本地 PowerShell 会话中照常编写脚本，而不会降低 Azure Stack 的安全状况。 请尽情享受其中的乐趣！
