...
对于生产环境，我们建议为每个终结点生成单个证书，并将其复制到相应的目录中。 对于开发环境，证书可以作为单个通配符证书提供，涵盖将 "使用者" 和 "使用者备用名称（SAN）" 字段中的所有命名空间复制到所有目录中。 涵盖所有终结点和服务的单个证书是一种不安全的状况，因此仅用于开发。 请记住，这两个选项都要求将通配符证书用于**acs**和 Key Vault 所需的终结点。
...
下表描述了资源提供程序所需的终结点和证书。 不需要将这些证书复制到 Azure Stack 中心部署文件夹中。 而是在资源提供程序安装过程中提供这些证书。
...
|事件中心|事件中心|&#42;eventhub. *&lt;区域 >&lt;fqdn >* （SAN）| eventhub. *&lt;fqdn >&lt;区域 >* |

<sup>1</sup>需要一个具有多个通配符使用者可选名称的证书。 并非所有公共证书颁发机构都支持在单个证书上使用多个通配符 San。

&#42; <sup>2</sup> appservice。 *&lt;区域 >。&lt;fqdn >* 通配符证书不能用于替代这三个证书（&lt;>&lt; *>&lt;>* *&lt;>&lt;* > *&lt;>* 。 Appservice 显式要求对这些终结点使用单独的证书。
