...
1. 准备要验证的包。 这是一个 `.zip` 文件，其内容必须与 [创建 OEM 包](azure-stack-vaas-create-oem-package.md)中所述的结构相匹配。
    ...
    > [!NOTE]
    > 请确保将 `.zip` 内容放在文件的根目录下 `.zip` 。 包中不应有子文件夹。

1. 在 [Azure 门户](https://portal.azure.com)中，选择 "包" 容器，然后在菜单栏中选择 " **上传** " 上传包。

1. 选择 `.zip` 要上传的包文件。 保留 **blob 类型** (的默认值，即 **块 Blob**) 和 **块大小**。
...
1. 在 [Azure 门户](https://portal.azure.com/)中，转到你的存储帐户，并导航到 `.zip` 包含你的包的。
...
1. 修改生成的 SAS URL，使其包含包的容器、 `{containername}` 和包 blob 的名称 `{mypackage.zip}` 。 喜欢这个： `https://storageaccountname.blob.core.windows.net/{containername}/{mypackage.zip}?sv=2016-05-31&ss=b&srt=co&sp=rl&se=2017-05-11T21:41:05Z&st=2017-05-11T13:41:05Z&spr=https`
...
> [!CAUTION]
> 此选项将为匿名只读访问打开 blob (s) 。

1. 将包容器的访问级别设置为 " **Blob** " 或 " **容器**"。 有关详细信息，请参阅[授予对容器和 Blob 的匿名用户权限](/azure/storage/storage-manage-access-to-resources#grant-anonymous-users-permissions-to-containers-and-blobs)。
...
8. `RequireDigitalSignature` -如果需要将包设置为 Microsoft 签名 () 运行 OEM 验证工作流，请提供值 **true** 。 如果要在最新版 Azure Stack 中心更新上验证 Microsoft 签名包，请将此值设置为 false (每月 Azure Stack 中心更新验证) 运行。
...
若要提交包签名请求，请向发送一封电子邮件， [vaashelp@microsoft.com](mailto:vaashelp@microsoft.com) 其中包含与此运行关联的解决方案名称和包验证名称。
