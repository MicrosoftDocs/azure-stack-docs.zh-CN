...
ms.locfileid: "81661417"
---
# <a name="validate-oem-packages"></a>验证 OEM 程序包

[!INCLUDE [Azure_Stack_Partner](./includes/azure-stack-partner-appliesto.md)]

如果已对固件或驱动程序进行了更改以完成解决方案验证，则可以测试新的原始设备制造商（OEM）包。 当你的包已通过测试时，它由 Microsoft 进行签名。 你的测试必须包含更新后的 OEM 扩展程序包，其中包含已通过 Windows Server 徽标和 PCS 测试的驱动程序和固件。
...
使用**包验证**工作流验证包时，需要提供**Azure 存储 blob**的 URL。 此 blob 是在更新过程中安装的测试签名 OEM 包。 使用在安装过程中创建的 Azure 存储帐户创建 blob （请参阅[将验证设置为服务 [VaaS] 资源](azure-stack-vaas-set-up-resources.md)）。

### <a name="prerequisite-provision-a-storage-container"></a>必备组件：预配存储容器

在存储帐户中为包 blob 创建容器。 此容器可用于所有包验证运行。

1. 在[Azure 门户](https://portal.azure.com)中，请参阅[设置 VaaS 资源](azure-stack-vaas-set-up-resources.md)中创建的存储帐户。
...
1. 准备要验证的包。 这是一个`.zip`文件，其内容必须与[创建 OEM 包](azure-stack-vaas-create-oem-package.md)中所述的结构相匹配。
    ...
    > [!NOTE]
    > 请确保将`.zip`内容放在`.zip`文件的根目录下。 包中不应有子文件夹。

1. 在[Azure 门户](https://portal.azure.com)中，选择 "包" 容器，然后在菜单栏中选择 "**上传**" 上传包。
...
如果存储容器的访问级别设置为 "**专用**"，则使用此选项，其中容器不会启用对容器或其 blob 的公共读取访问权限。

> [!NOTE]
> 此方法对于*交互式*测试不起作用。 请参阅[选项2：构造容器 SAS URL](#option-2-construct-a-container-sas-url)。

1. 在[Azure 门户](https://portal.azure.com/)中，转到你的存储帐户，并导航`.zip`到包含你的包的。
...
1. 修改生成的 SAS URL，使其包含包的`{containername}`容器、和包 blob 的名称`{mypackage.zip}`。 喜欢这个：`https://storageaccountname.blob.core.windows.net/{containername}/{mypackage.zip}?sv=2016-05-31&ss=b&srt=co&sp=rl&se=2017-05-11T21:41:05Z&st=2017-05-11T13:41:05Z&spr=https`
...
1. 将包容器的访问级别设置为 " **Blob** " 或 "**容器**"。 有关详细信息，请参阅[授予对容器和 Blob 的匿名用户权限](https://docs.microsoft.com/azure/storage/storage-manage-access-to-resources#grant-anonymous-users-permissions-to-containers-and-blobs)。
...
8. `RequireDigitalSignature`-如果需要将包设置为 Microsoft 签名（运行 OEM 验证工作流），请提供值**true** 。 如果要在最新的 Azure Stack 中心更新上验证 Microsoft 签名的包，请将此值设置为 false （每月 Azure Stack 中心更新验证运行）。
...
1. 在 "**包验证测试摘要**" 页中，你将运行适用于你的方案的所列测试的子集。
    ...
    > [!NOTE]
    > 在现有实例上计划验证测试会在门户中创建一个新实例来代替旧实例。 旧实例的日志将保留，但无法从门户进行访问。<br><br>
...
若要提交包签名请求，请向发送一[vaashelp@microsoft.com](mailto:vaashelp@microsoft.com)封电子邮件，其中包含与此运行关联的解决方案名称和包验证名称。
