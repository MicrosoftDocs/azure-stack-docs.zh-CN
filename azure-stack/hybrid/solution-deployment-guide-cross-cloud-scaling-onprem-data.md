...
- Azure Stack 中心安装应安装以下各项：
  - Azure 应用服务。 使用 Azure Stack 中心操作员在你的环境中部署和配置 Azure App Service。 在本教程中，应用服务必须至少有一 (1) 个可用的专用辅助角色。
...
3. 在“市场”中选择“计算”，然后选择“更多”。 在 "**详细信息**" 下，选择**免费 SQL Server 许可证： Windows Server 上的 SQL Server 2017 开发人员**映像。
...
5. 在“基本信息”>“配置基本设置”中，提供虚拟机 (VM) 的**名称**、SQL Server SA 的**用户名**，以及 SA 的**密码**。  在“订阅”下拉列表中，选择要部署到的订阅。 对于 "**资源组**"，请使用 "**选择现有**"，并将 VM 放在与 Azure Stack 中心 web 应用相同的资源组中。
...
Azure 应用服务简化了运行和管理 Web 应用的过程。 由于 Azure Stack 中心与 Azure 一致，因此应用服务可以在这两种环境中运行。 你将使用应用服务来托管应用。
...
若要在 Azure 中的 web 前端和 Azure Stack 中心的 SQL Server 数据库之间提供连接，必须将 web 应用连接到 Azure 与 Azure Stack 中心之间的混合网络。 若要启用连接，必须：
...
通过对跨云应用正确配置 DNS，用户可以访问 web 应用的全局 Azure 和 Azure Stack 中心实例。 本教程中的 DNS 配置还可让 Azure 流量管理器在负载增加或减少时路由流量。
...
1. 通过**将 CNAME 映射到 Azure 应用服务**，将 [app.northwind.com](https://docs.microsoft.com/Azure/app-service/app-service-web-tutorial-custom-domain#map-a-cname-record) 主机名添加到 Azure Web 应用。

### <a name="configure-custom-domains-in-azure-stack-hub"></a>在 Azure Stack 中心配置自定义域

1. 通过将[A 记录映射到 Azure App Service](https://docs.microsoft.com/Azure/app-service/app-service-web-tutorial-custom-domain#map-an-a-record)，将**azurestack.northwind.com**主机名添加到 Azure Stack 中心 web 应用。 对应用服务应用请使用可通过 Internet 路由的 IP 地址。

2. 通过将[CNAME 映射到 Azure App Service](https://docs.microsoft.com/Azure/app-service/app-service-web-tutorial-custom-domain#map-a-cname-record)，将**app.northwind.com**主机名添加到 Azure Stack 中心 web 应用。 使用在前一步骤 (1) 中配置的主机名作为 CNAME 的目标。
...
2. 在 Azure 中，按照**将现有的自定义 SSL 证书绑定到 Azure Web 应用**一文的“准备 Web 应用”和“绑定 SSL 证书”部分的说明操作。[](https://docs.microsoft.com/Azure/app-service/app-service-web-tutorial-custom-ssl) 为“SSL 类型”选择“基于 SNI 的 SSL”。

3. 将所有流量重定向到 HTTPS 端口。 遵照**将现有的自定义 SSL 证书绑定到 Azure Web 应用**一文的“强制实施 HTTPS”部分的说明操作。[](https://docs.microsoft.com/Azure/app-service/app-service-web-tutorial-custom-ssl)
...
2. 向项目中[添加 Application Insights](https://docs.microsoft.com/azure/azure-monitor/app/asp-net-core#enable-client-side-telemetry-for-web-applications)，以传输在 Web 流量增加或减少时 Application Insights 用于创建警报的遥测。

### <a name="configure-dynamic-connection-strings"></a>配置动态连接字符串

Web 应用的每个实例都会使用不同的方法连接到 SQL 数据库。 Azure 中的应用使用 SQL Server 虚拟机（VM）的专用 IP 地址，Azure Stack 中心的应用使用 SQL Server VM 的公共 IP 地址。
...
1. 为 Azure 和 Azure Stack 集线器创建连接字符串。 在这些字符串中，除使用的 IP 地址外，其余部分应该相同。
...
3. 在“自动缩放设置名称”中输入名称。 对于“默认”自动缩放规则，请选择“基于指标缩放”。 将**实例限制**设置为**最小值： 1**，**最大值： 10**，**默认值为 1**。
...
当流量减少时，Azure Web 应用可以自动减少活动实例的数目，以降低成本。 此操作不如缩小，并最大程度地降低对应用用户的影响。
