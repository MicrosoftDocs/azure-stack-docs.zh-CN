...
Azure Stack 操作员必须部署应用服务、 创建计划和产品/服务、 创建租户订阅，并添加 Windows Server 2016 映像。 如果已有这些组件，请确保在开始此解决方案之前，它们符合的要求。
...
### <a name="azure"></a>Azure
 ...
 - 创建[web 应用](https://docs.microsoft.com/vsts/build-release/apps/cd/azure/aspnet-core-to-azure-webapp?view=vsts&tabs=vsts)在 Azure 中。 记下的 web 应用 URL，因为需要使用该解决方案中。
...
在此解决方案中的示例使用以下值。 可使用这些值创建测试环境，或根据这些值来更好地理解示例。 详细了解 VPN 网关设置一般情况下，请参阅[关于 VPN 网关设置](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpn-gateway-settings)。
...
请执行以下步骤，以便通过门户创建虚拟网络。 您可以使用这些[示例值](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal#values)如果使用本文作为仅一种解决方案。 如果使用本文来配置生产环境中，替换你自己的值的示例设置。
...
4. 使用**教程示例值**，在“创建本地网关”上指定本地网关的值  。 包括以下其他值。
    ...
    - **地址空间**： 他地址为此本地网络代表网络的范围。 可以添加多个地址空间范围。 请确保指定的范围没有与要连接到的其他网络的范围重叠。 Azure 会将指定的地址范围路由到本地 VPN 设备 IP 地址。 如果需要连接到本地站点，请使用自己的值而不是示例值。
