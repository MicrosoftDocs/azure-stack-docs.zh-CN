...
4. 创建一个服务主体，使用客户端机密，使用**订阅**作用域和**所有者**角色。 保存服务主体的 ID 和机密。 有关创建 Azure Stack 的服务主体的信息，请参阅[使用应用程序标识来访问资源](../operator/azure-stack-create-service-principals.md)。 现已设置 Azure Stack 环境。

5. 在代码中从 Go SDK 配置文件导入服务模块。 Azure Stack 配置文件的当前版本是**2019年-03-01**。 例如，若要导入网络模块从**2019年-03-01**配置文件类型，请使用以下代码：
...
1. 如果可以使用订阅中具有所有者角色的某个服务主体，请跳过此步骤。 否则请参阅[使用应用程序标识来访问资源](../operator/azure-stack-create-service-principals.md)有关创建服务主体使用客户端机密，并将其分配到你的订阅范围内的"所有者"角色的说明。 请确保捕获的服务主体应用程序 ID 和密码。
...
4. 添加 `main` 方法。 `main` 方法首先使用上一步骤中定义的方法获取令牌。 然后，该方法使用配置文件中的网络模块创建客户端。 最后创建虚拟网络。
   ...
   ```
下面是一些可用于 Azure Stack 使用 Go SDK 的代码示例：
...
- [使用托管磁盘](https://github.com/Azure-Samples/Hybrid-Compute-Go-ManagedDisks)、 （示例使用 2019年-03-01 配置文件的目标的最新的 API 版本支持的 Azure Stack）
