---
title: SQL Server 的 Azure Stack 上的 Windows N 层应用程序 |Microsoft Docs
description: 了解如何使用 SQL Server 在 Azure Stack 上运行 Windows N 层应用程序。
...
ms.locfileid: "73569102"
---
# <a name="windows-n-tier-application-on-azure-stack-with-sql-server"></a>具有 SQL Server Azure Stack 上的 Windows N 层应用程序
...
-   **DNS**。 Azure Stack 没有提供自己的 DNS 宿主服务，因此请使用添加中的 DNS 服务器。
...
定义用于将网络流量定向到 VM 的负载均衡器规则。 例如，若要启用 HTTP 流量，请将前端配置中的端口 80 映射到后端地址池上的端口 80。 当客户端向端口80发送 HTTP 请求时，负载均衡器将使用包含源 IP 地址的[哈希算法](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview#fundamental-load-balancer-features)选择后端 IP 地址。 客户端请求将在后端地址池中的所有 VM 之间分配。
...
对于 SQL 性能优化，还可以参阅[sql server 最佳实践一文，以优化 Azure Stack 中的性能](https://docs.microsoft.com/azure-stack/user/azure-stack-sql-server-vm-considerations)。
...
有关详细信息，请参阅[规模集的设计注意事项](https://docs.microsoft.com/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-design-overview)。 此设计注意事项主要适用于 Azure Stack，但有一些注意事项：

-   Azure Stack 上的虚拟机规模集不支持过度预配或滚动升级。

-   无法在 Azure Stack 上自动缩放虚拟机规模集。

-   强烈建议在虚拟机规模集的 Azure Stack 而不是非托管磁盘上使用托管磁盘

-   目前 Azure Stack 上存在 700 VM 限制，适用于所有 Azure Stack 基础结构 Vm、单个 Vm 和规模集实例的帐户。

## <a name="subscription-limits"></a>订阅限制

每个 Azure Stack 租户订阅都有默认限制，包括 Azure Stack 操作员配置的每个区域的最大 Vm 数。 有关详细信息，请参阅[Azure Stack 服务、计划、产品/服务、订阅概述](https://docs.microsoft.com/azure-stack/operator/service-plan-offer-subscription-overview)。 另请参阅[Azure Stack 中的配额类型](https://docs.microsoft.com/azure-stack/operator/azure-stack-quota-types)。
