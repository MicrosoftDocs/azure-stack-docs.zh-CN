...
2. 创建部署 VM 和自定义脚本扩展。
    ...
    -  运行 DVM 自定义脚本。 该脚本执行以下任务：
        ...
        3. 加载 AKS 引擎的 API 模型。
        4. 将 AKS 引擎部署到 Kubernetes 群集并将 Azure Stack 的云配置文件保存`/etc/kubernetes/azurestackcloud.json`到。
3. 创建主 VM。
...
4.  如果 VM **正常**，请评估 DVM。 如果 DVM 有一条错误消息：
    ...
    - 公钥可能无效。 查看创建的密钥。  
    - 请与 Azure Stack 管理员联系, 使用特权终结点检索 Azure Stack 的日志。 有关详细信息，请参阅 [Azure Stack 诊断工具](../operator/azure-stack-configure-on-demand-diagnostic-log-collection.md#using-pep)。
