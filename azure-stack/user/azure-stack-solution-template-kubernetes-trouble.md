...
1. 从市场项收集输入参数。
    ...
    -  **服务主体**： Kubernetes Azure 云提供程序使用的 ID。 创建服务主体时标识为应用程序 ID 的客户端 ID。 
    -  **客户端密钥**：创建服务主体时创建的密钥。
...
3. 查找脚本所需的信息并运行该脚本：
    ...
    | -h、--help  | 打印命令用法。 | |
    | -i、--identity-file | 创建 Kubernetes 群集时，传递给 marketplace 项的 RSA 私钥文件的路径。 需要将远程登录到 Kubernetes 节点。 | C:\data\ id_rsa （Putty）<br>~/.ssh/id_rsa （SSH）
