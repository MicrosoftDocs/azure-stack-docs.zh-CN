...
Azure Stack 上的磁盘 IOPS（每秒输入/输出操作次数）是与 VM 大小相关的函数，与磁盘类型无关。 这意味着，对于 Standard_Fs 系列 VM，不管你选择 SSD 还是 HDD 作为磁盘类型，单个额外的数据磁盘的 IOPS 限制都是 2300。 施加的 IOPS 限制是一种上限（最大可能值），目的是防止邻域干扰。 这并不能保证你会获得特定 VM 大小的 IOPS。

## <a name="vm-general-purpose"></a>VM 常规用途

常规用途 VM 大小提供均衡的 CPU 与内存之比。 它们用于测试和开发、小到中型数据库和低到中等流量的 web 服务器。 对于高级 VM 大小，每个数据磁盘是 2300 IOPS，基本 A 系列除外。 对于基本 A 系列，数据磁盘大小为 500 IOPS。
...
|Size     |vCPU     |内存 (GiB) | 临时存储 (GiB)  | 最大 OS 磁盘吞吐量 (IOPS) | 最大临时存储吞吐量 (IOPS) | 最大数据磁盘数/吞吐量 (IOPS) | 最大 NIC 数 |    
|----------------|--|------|----|----|----|-------|---------|
