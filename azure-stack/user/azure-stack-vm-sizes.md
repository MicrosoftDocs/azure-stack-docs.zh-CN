...
一般用途的 VM 大小提供平衡的 CPU 与内存之比。 它们用于测试和开发、小到中型数据库和低到中等流量的 web 服务器。 每个数据磁盘为高级 VM 大小 2300 IOPS，基本 A 系列除外。 对于基本 A，数据磁盘大小为 500 IOPS。
...
内存优化 VM 大小为关系数据库服务器、中到大型缓存和内存中分析提供了高内存与 CPU 的比率。
