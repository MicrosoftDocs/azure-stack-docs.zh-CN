...
安装在 IaaS VM 中的操作系统会进行限制，确保只有特定的产品可以用来保护其所包含的数据。 对于 Windows 基于 IaaS Vm，可以使用 Microsoft 和合作伙伴产品来保护数据。 对于基于 Linux 的 IaaS VM，唯一的选择是使用合作伙伴产品。 请参阅[此数据表，了解其产品适用于 Azure Stack 的所有 BC/DR 合作伙伴](https://aka.ms/azurestackbcdrpartners)。
