...
另一指标是*平均恢复时间*（MTTR），这是在发生故障后还原应用程序所花的平均时间。 MTTR 是系统的内部价值。 如果 MTTR 超过 RTO，则系统中的故障会导致无法接受的业务中断，因为无法在定义的 RTO 内还原系统。
...
你的环境中的某些应用可能不需要防范计划外停机或数据丢失。 例如，用于开发和测试的 Vm 通常不需要恢复。 您决定在不保护应用程序或数据集的情况下执行此操作。
