...
本文介绍如何使用[Azure 存储资源管理器](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer)连接到 Azure Stack 订阅和存储帐户。 存储资源管理器是一款独立应用，可用于在 Windows、macOS 和 Linux 上轻松处理 Azure Stack 存储数据。

> [!NOTE]  
> 有多种工具可用于将数据移入和移出 Azure Stack 存储。 有关详细信息，请参阅[用于 Azure Stack 存储的数据传输工具](azure-stack-storage-transfer.md)。

如果尚未安装，请[下载](https://www.storageexplorer.com/)并安装存储资源管理器。

连接到 Azure Stack 订阅或存储帐户后，可以使用[Azure 存储资源管理器文章](/azure/vs-azure-tools-storage-manage-with-storage-explorer)来处理 Azure Stack 数据。 

## <a name="prepare-for-connecting-to-azure-stack"></a>准备连接到 Azure Stack

需要直接访问 Azure Stack 或 VPN 连接，存储资源管理器才能访问 Azure Stack 订阅。 若要了解如何设置到 Azure Stack 的 VPN 连接，请参阅[使用 VPN 连接到 Azure Stack](../asdk/asdk-connect.md#connect-to-azure-stack-using-vpn)。

> [!Note]  
> 对于 ASDK，如果要通过 VPN 连接到 ASDK，请不要使用在 VPN 安装过程中创建的根证书（）。  这是 DER 编码的证书，不允许存储资源管理器检索 Azure Stack 订阅。 使用以下步骤导出要与存储资源管理器一起使用的64编码证书。
...
导出并导入断开连接的集成系统和 ASDK 的 Azure Stack 证书。 对于连接的集成系统，证书已公开签名，无需执行此步骤。

1. 在 Azure Stack 主机上打开 `mmc.exe`，或在本地计算机上打开与 Azure Stack 的 VPN 连接。 
...
8. 存储资源管理器重新启动后，选择 "**编辑**" 菜单，并查看是否选择了 "**目标 Azure Stack api** "。 如果没有，请选择 "**目标 Azure Stack**"，然后重新启动存储资源管理器以使更改生效。 此配置是必需的，否则无法与 Azure Stack 环境兼容。
...
3. 在 "连接到 Azure 存储" 对话框中的 " **azure 环境**" 下，选择 " **Azure**、 **azure 中国世纪互联**、 **azure 德国**、 **azure 美国政府**" 或 "**添加新环境**"。 这取决于所使用的 Azure Stack 帐户。 选择 "**登录**" 以登录到与至少一个活动 Azure Stack 订阅关联的 Azure Stack 帐户。
...
> [!Note]  
> Azure 联合身份验证服务（AD FS）登录体验支持 Azure Stack 1804 或更新版本的存储资源管理器1.2.0 或更新版本。
使用以下步骤将存储资源管理器连接到属于 AD FS 帐户的 Azure Stack 订阅。

1. 选择 "**管理帐户**"。 资源管理器将列出你登录到的 Microsoft 订阅。
2. 选择 "**添加帐户**" 以连接到 Azure Stack 订阅。
...
4. 输入 Azure Stack 自定义环境所需的信息。 
    ...
    如果你正在 Azure Stack 集成系统，但不知道管理终结点，请联系你的操作员。

    ![添加帐户自定义环境](./media/azure-stack-storage-connect-se/custom-environments.png)

5. 选择 "**登录**" 以连接到与至少一个活动 Azure Stack 订阅关联的 Azure Stack 帐户。



6. 选择要使用的 Azure Stack 订阅，并选择 "**应用**"。
...
## <a name="connect-to-an-azure-stack-storage-account"></a>连接到 Azure Stack 的存储帐户

你还可以使用存储帐户名称和密钥对连接到 Azure Stack 的存储帐户。
...
4. 在**帐户名称**中输入帐户名称，并将帐户密钥粘贴到 "**帐户密钥**" 文本框中。 然后，在 "**存储终结点域**" 中选择 "**其他" （在下面输入）** 并输入 Azure Stack 终结点。
    ...
    Azure Stack 终结点包含两部分：区域名称和 Azure Stack 域。 在 Azure Stack 开发工具包中，默认终结点是**test-azurestack**。 如果你不确定终结点，请与云管理员联系。
