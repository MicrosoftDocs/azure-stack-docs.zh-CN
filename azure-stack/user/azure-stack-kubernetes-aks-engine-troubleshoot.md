...
description: 本主题包含 Azure Stack 上的 AKS 引擎的故障排除步骤。
...
可以访问由 AKS 引擎创建的查看信息。 当应用程序运行时，AKS 引擎会报告状态和错误。 可以通过管道将输出传递给文本文件，也可以直接从命令行控制台复制。
...
2. 查找`getkuberneteslogs.sh`脚本所需的参数。 此脚本将使用以下参数：
    ...
    |   -n、--user-namespace               | 从指定命名空间中的容器收集日志（始终收集 kube 系统日志） | 否 |   监视 |
