...
ms.locfileid: "67816238"
---
# <a name="azure-stack-managed-disks-differences-and-considerations"></a>Azure Stack 托管磁盘：差异与注意事项

本文汇总了之间的差异[托管在 Azure Stack 中的磁盘](azure-stack-manage-vm-disks.md)并[托管在 Azure 中的磁盘](/azure/virtual-machines/windows/managed-disks-overview)。 有关 Azure Stack 与 Azure 之间的大致差异的详细信息，请参阅[重要注意事项](azure-stack-considerations.md)一文。

托管的磁盘通过管理简化 IaaS 虚拟机 (Vm) 的磁盘管理[存储帐户](../operator/azure-stack-manage-storage-accounts.md)与 VM 磁盘关联。

> [!NOTE]  
> Azure Stack 上的托管的磁盘是 1808年更新从开始提供。 从 1811年更新开始，它是默认情况下启用创建使用 Azure Stack 门户的 Vm 时。
...
|磁盘快照复制|Azure 托管磁盘附加到正在运行的 VM 支持的快照|尚不支持 |
...
> [!NOTE]  
> Azure PowerShell cmdlet **Convertto-azurermvmmanageddisk**不能用于将非托管的磁盘转换为 Azure Stack 中的托管磁盘。 Azure Stack 目前不支持此 cmdlet。
...
> [!NOTE]
> 请务必将 VM 通用化。 从从未正确通用化映像创建虚拟机将导致**VMProvisioningTimeout**错误。
...
请确保将 VM 通用化正确执行此步骤前。 通用化之后，不再可以使用此 VM。 从从未正确通用化映像创建虚拟机将导致**VMProvisioningTimeout**错误。

按照中的说明[从存储帐户中的 VHD 创建映像](/azure/virtual-machines/windows/capture-image-resource#create-an-image-from-a-vhd-in-a-storage-account)从存储帐户中的通用 VHD 创建托管的映像。 您可以在将来使用此映像创建托管的 Vm。

#### <a name="case-2-create-managed-vm-from-managed-image-using-powershell"></a>案例 2：使用 PowerShell 基于托管映像创建托管 VM

之后你从创建映像的现有托管磁盘 VM 使用中的脚本[从托管磁盘使用 PowerShell 创建映像](/azure/virtual-machines/windows/capture-image-resource#create-an-image-from-a-managed-disk-using-powershell)，使用以下示例脚本从现有的图像对象创建类似的 Linux VM。
...
也可以使用门户基于托管映像创建 VM。 有关详细信息，请参阅 Azure 托管映像文章[在 Azure 中创建通用 VM 的托管的映像](/azure/virtual-machines/windows/capture-image-resource)并[从托管映像创建 VM](/azure/virtual-machines/windows/create-vm-generalized-managed)。

## <a name="configuration"></a>配置

应用 1808年后更新或更高版本，则必须进行以下配置更改之前使用托管的磁盘：
