---
title: 如何在 Azure Stack 中通过 Commvault 备份 VM |Microsoft Docs
description: 了解如何在 Azure Stack 上通过 Commvault 备份 VM。
...
本文逐步讲解如何配置 Commvault 实时同步，以更新位于单独 Azure Stack 缩放单位上的恢复 VM。 本文详细介绍如何配置常见合作伙伴解决方案来保护和恢复 Azure Stack 上部署的虚拟机的数据和系统状态。
...
2. 在辅助位置中创建存储帐户。 本文假设你将在单独的 Azure Stack 实例（目标）中的存储帐户中创建一个 Blob 容器，并且可从源 Azure Stack 访问目标 Azure Stack。

3. 在源 Azure Stack 实例上配置 Commvault，并将源 Azure Stack 中的 Vm 添加到 VM 组。

4. 配置 Commvault 的 LifeSync。

你还可以下载并提供兼容的合作伙伴 VM 映像，以保护 Azure Stack Vm 与 Azure 云或其他 Azure Stack。 本文将介绍如何通过 Commvault 实时同步来保护 VM。
...
3. 配置 "创建虚拟机" 中的基本设置 **，1个基础知识**：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 输入“名称”。
    ...
    h. 选择 Azure Stack 的**位置**。 如果使用的是 ASDK，请选择 "**本地**"。
...
5. 选择 Commvault VM 的设置。
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 将可用性设置为**None**。
...
你需要了解你的身份管理器是 Azure AD 还是 AD DFS。 下表包含在 Azure Stack 中设置 Commvault 时所需的信息。

| 元素 | 描述 | Source |
|--------------------------|--------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| Azure 资源管理器 URL | Azure Stack 资源管理器终结点。 | https://docs.microsoft.com/azure-stack/user/azure-stack-version-profiles-ruby?view=azs-1908#the-azure-stack-resource-manager-endpoint |
| 应用程序名称 |  |  |
...
| 订阅 ID | 使用订阅 ID 可以访问 Azure Stack 中的产品/服务。 | https://docs.microsoft.com/azure-stack/operator/service-plan-offer-subscription-overview?view=azs-1908#subscriptions |
| 租户 ID （目录 ID） | 你的 Azure Stack 租户 ID。 | https://docs.microsoft.com/azure-stack/operator/azure-stack-identity-overview?view=azs-1908 |
...
4. 将备份存储库配置为使用 Commvault Commcell 控制台中 Azure Stack 的外部存储。 在 CommCell 浏览器中，选择 "存储资源" > 存储池 "。 右键单击并选择 "**添加存储池"。** 选择“云”。
...
7. 选择云服务提供商。 在此过程中，我们将在不同的位置使用另一个 Azure Stack。 选择 Microsoft Azure 存储。
...
10. 按照[创建 Microsoft Azure Stack 客户端](https://documentation.commvault.com/commvault/v11_sp13/article?p=86495.htm)中的说明创建 Microsoft Azure Stack 的客户端
...
有两个选项可用。 你可以选择从备份的主要副本复制更改，或将更改从辅助副本复制到恢复 VM。 从备份集进行复制可消除对源计算机的读取 IO 影响。

1. 在配置实时同步期间，你将需要提供源 Azure Stack （虚拟服务器代理）和目标 Azure Stack 详细信息。

2. 有关配置 Commvault 实时同步的步骤，请参阅[Microsoft Azure Stack 的实时同步复制](https://documentation.commvault.com/commvault/v11_sp13/article?p=94386.htm)。

    ![](./media/azure-stack-network-howto-backup-commvault/live-sync-1.png)
 
3. 在配置实时同步期间，你将需要提供目标 Azure Stack 和虚拟服务器代理详细信息。
...
Commvault 实时同步可让你将计算机从一个 Azure Stack 故障转移到另一个，并故障回复以恢复原始 Azure Stack 上的操作。 工作流是自动执行和记录的。

![](./media/azure-stack-network-howto-backup-commvault/back-up-live-sync-panel.png)

选择要故障转移到恢复 Azure Stack 的 Vm，然后选择计划内或计划外的故障转移。 当在恢复站点中恢复操作之前有时间正常关闭生产环境时，计划的故障转移是合适的。 计划的故障转移将关闭生产 Vm，将最后的更改复制到恢复站点，并使恢复 Vm 以最新数据联机，并应用在实时同步配置期间指定的 VM 大小和网络配置。 非计划的故障转移将尝试关闭生产 Vm，但在生产环境不可用时将继续，并只需将上次接收到的复制数据集与应用到 VM 的数据和大小和网络先前所选的配置。 下图演示了一个未计划的故障转移，其中恢复 Vm 已通过 Commvault 实时同步进入联机状态。
