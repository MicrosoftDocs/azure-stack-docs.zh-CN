...
从版本 1808年，Azure Stack 支持在虚拟机，操作系统 (OS) 和数据磁盘作为托管的磁盘和非托管的磁盘的使用。 在版本 1808 之前，仅支持非托管磁盘。
...
5. 在中**创建托管的磁盘**边栏选项卡：
   ...
   * 选择“帐户类型”。
      ...
      **标准 HDD**  
      标准磁盘 (HDD) 基于磁驱动器，适用于不经常访问数据的应用程序。 区域冗余磁盘由跨多个区域复制数据的区域冗余存储 (ZRS) 支持，都可用，即使单个区域出现故障。

   * 选择“源类型”。

     从其他磁盘快照、存储帐户的 BLOB 中创建磁盘，或创建空磁盘。

      **快照**：选择一个快照（如果有）。 该快照必须在 VM 的订阅和位置中可用。

      **存储 blob**:
     * 添加包含磁盘映像的存储 Blob 的 URI。  
     * 选择“浏览”打开“存储帐户”边栏选项卡。 有关说明，请参阅[从存储帐户中添加数据磁盘](#add-a-data-disk-from-a-storage-account)。
...
6. Azure Stack 创建磁盘并将其附加到虚拟机后下的虚拟机磁盘设置中列出的新磁盘**数据磁盘**。
...
第一个命令创建一个虚拟机对象，然后将其存储在`$VirtualMachine`变量。 该命令将分配给虚拟机的名称和大小：
...
以下示例使用 PowerShell 命令将三个数据磁盘添加到现有的 VM。 第一个命令获取名为的虚拟机**VirtualMachine**通过使用**Get-azurermvm** cmdlet。 该命令将存储中的虚拟机`$VirtualMachine`变量：
...
最后一个命令将更新的虚拟机存储在状态`$VirtualMachine`在`-ResourceGroupName`:
