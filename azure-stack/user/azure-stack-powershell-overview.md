...
每个接口联系全球 Azure 中的资源提供程序或 Azure Stack。 资源提供程序启用 Azure 功能。 例如，Azure 计算资源提供程序可让你以编程方式访问虚拟机及其支持资源的创建和管理。
...
以下框图显示了 PowerShell 模块集之间的关系。 在计算机上，可以加载 PowerShell 模块并管理全局 Azure 和 Azure Stack。
...
Azure Stack 向云操作员公开一组资源提供程序用于安装和维护 Azure Stack。 在全球 Azure 中，这种交互是从用户抽象的，并在后台作为 Azure 的一部分进行处理。 但是，企业可以使用 Azure Stack 来支持私有云。 若要执行这些任务，操作员需要与 Azure Stack 管理 API 交互。 有关详细信息，请参阅[安装适用于 Azure Stack 的 PowerShell](../operator/azure-stack-powershell-install.md)。
...
在长时间使用 Azure Stack PowerShell 的过程中，你会发现需要反复安装模块。 如果你同时使用全局 Azure，此例程可能会很困难，因为你需要卸载并重新安装模块，具体取决于你的目标。 
