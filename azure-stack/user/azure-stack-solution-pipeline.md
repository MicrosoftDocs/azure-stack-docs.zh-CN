...
- 根据以下说明部署并配置 Azure Stack 集成系统或 Azure Stack 开发工具包（ASDK）。 
  ...
  1. 在 Azure Stack 中创建[计划和产品/服务](../operator/service-plan-offer-subscription-overview.md)。
...
- **环境 URL**：如果未选择**AzureCloud**，请输入你的环境 URL，如*Https @no__t：/test-azurestack*。
...
- **Connection：未验证 @ no__t-0：选择 "**验证连接**" 以验证服务主体的连接设置。
...
1. 在 Visual Studio**解决方案资源管理器**中，打开*WebApplication*文件并添加 @no__t。 有关此步骤的详细信息，请参阅[自包含部署](/dotnet/core/deploying/#self-contained-deployments-scd)。
...
1. 在 Azure DevOps 项目中，在左侧导航栏中选择 "**管道**@no__t **-1"** ，然后选择 "**新建管道**"。 
