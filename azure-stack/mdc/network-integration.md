...
边缘交换机要求第3层上行链路具有点到点 Ip (/30 网络) 在物理接口上进行配置。 第2层具有边缘交换机的上行链路支持 Azure Stack 操作。 

### <a name="bgp-routing"></a>BGP 路由

使用 BGP 等动态路由协议可以保证系统始终会注意到网络更改和便于管理。 为增强安全性，可以在边缘和边框之间的 BGP 对等互连上设置密码。

如下图所示，将使用前缀列表阻止播发 TOR 交换机上的专用 IP 空间。 前缀列表拒绝专用网络的播发，并将其作为 TOR 与边缘之间的连接的路由映射。
...
若要使用静态路由将 Azure Stack 集成到你的网络环境中，则必须连接边框与边缘设备之间的所有四个物理链接。 由于静态路由的工作方式，无法保证高可用性。

必须配置边框设备，使其包含指向四个 P2P Ip 之间的每一个 Ip 的路由，该路由指向在 Azure Stack 中的任何网络的流量之间设置的每个 P2P Ip 初始部署需要到 BMC 网络和外部网络的静态路由。 操作员可以选择在边框中保留静态路由，以访问位于 BMC 和基础结构网络上的管理资源。 添加指向交换机基础结构和交换机管理网络的静态路由是可选的。

TOR 设备配置有将所有流量发送到边界设备的静态默认路由。 默认规则的一个流量例外是，对于专用空间，将使用应用于 TOR 到边界连接的访问控制列表阻止该流量。

静态路由仅适用于边缘和边框开关之间的上行链路。
...
*\<NAMINGPREFIX\>- \<REGION\> ns01。\<EXTERNALDOMAINNAME\>*

*\<NAMINGPREFIX\>- \<REGION\> ns02。\<EXTERNALDOMAINNAME\>*
...
此信息在管理门户中提供，但也是在名为 *AzureStackStampInformation.js的*文件中的所有 Azure Stack 部署的末尾创建的。
此文件位于部署虚拟机的 " *C： \\ CloudDeployment \\ logs*"   文件夹中。 如果不确定对 Azure Stack 部署使用了什么值，可以从该文件中获取这些值。
...
如果你不熟悉如何使用 DNS 执行条件性转发，请参阅以下 TechNet 文章：为域名分配条件转发器或特定于你的 DNS 解决方案的文档。
...
DNS 转发器 IP 是在 Azure Stack 部署期间设置的。 但是，如果转发器 ip 出于任何原因需要更新，则可以通过连接到特权终结点并运行 *AzSDnsForwarder*   和 *AzSDnsForwarder [[-IPAddress] \<IPAddress[]\> ]* PowerShell cmdlet 来编辑这些值   。 有关详细信息，请参阅特权终结点。
...
这些 VIP 是从公共 IP 地址池分配的。 每个 VIP 受软件定义的网络层中的访问控制列表 (ACL) 保护。 还可以在物理交换机（TOR 和 BMC）之间使用 ACL 来进一步强化解决方案。 将会根据部署时的指定，针对外部 DNS 区域中的每个终结点创建一个 DNS 条目。 例如，为用户门户指定了门户的 DNS 主机条目。*\<region\>.\<fqdn\>*.
...
将 Azure Stack 终结点发布到外部网络需要一组基础结构 VIP。  * (VIP) 表中的终结点*   显示了每个终结点、所需的端口和协议。 请参阅特定资源提供程序部署文档，了解需要其他资源提供程序（例如 SQL 资源提供程序）的终结点。
